...
class Engravtor(gfx.WorldObject):
    def __init__(self,env_map = None):
        ...
        persp_camera.depth_range = (0.01,10000)
        persp_camera.show_pos(self.target.local.position,up=[0,0,1],depth=1.0)
    ...
    def get_view_focus(self):
        return self.camera.local.position,self.target.local.position
